...
def lambda_handler(event, context):
    ...
    try:
        ...
        diagnoses = run_query(
            f"SELECT icd10_cm_code, diagnosis_date, hcc_code, hcc_description "
            f"FROM diagnosis_history WHERE member_id = '{member_id}' "
            f"ORDER BY diagnosis_date DESC LIMIT 10")
        gaps = run_query(
            f"SELECT measure_name, gap_status, open_date, close_date "
            f"FROM hedis_gaps WHERE member_id = '{member_id}'")
        risk = run_query(
            f"SELECT risk_tier, risk_score_value, raf_score "
            f"FROM risk_scores WHERE member_id = '{member_id}' LIMIT 1")
        sdoh = run_query(
            f"SELECT barrier_type, severity, identified_date "
